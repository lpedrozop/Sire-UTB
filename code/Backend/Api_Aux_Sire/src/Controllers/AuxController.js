...
export const ObtenerEspaciosAux = async (req, res) => {
   if (hasRequiredDelegatedPermissions(req.authInfo, AuthConfig.protectedRoutes.reserva.delegatedPermissions.access)) {
       try {
           ...
           if (clases.length >= 0 && reservas.length === 0) {
               return res.status(200).json({"Reservas": "No hay reservas", "Clases_regulares": clases});
           }
           if (clases.length === 0 && reservas.length >= 0) {
               return res.status(200).json({"Reservas": reservas, "Clases_regulares": "No hay clases"});
           }
           else {
               return res.status(200).json({message: '¡Eso es todo por hoy! No cuenta con espacios asignados actualmente'});
           }
       }
    catch (e) {
